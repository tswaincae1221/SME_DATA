...
def build_sequences(
    ...
    ordered = frame.copy()
    ordered["Date"] = pd.to_numeric(ordered["Date"], errors="raise").astype(
        "int64"
    )
    ordered["STN_ID"] = pd.to_numeric(
        ordered["STN_ID"], errors="raise"
    ).astype("int64")
...
class NumpyLSTMRegressor:
    def __init__(self, input_size: int, hidden_size: int, output_size: int, seed: int):
        ...
        self.params = {
            "W": rng.uniform(
                -limit, limit, size=(fan_in, 4 * hidden_size)
            ).astype(np.float32),
            "b": np.zeros(4 * hidden_size, dtype=np.float32),
            "Wy": rng.normal(
                0.0, 1.0 / math.sqrt(hidden_size), size=(hidden_size, output_size)
            ).astype(np.float32),
    ...
    def forward(
        self, x: np.ndarray, *, keep_cache: bool
        ...
        caches: list[tuple[np.ndarray, ...]] | None = [] if keep_cache else None
        h = self.hidden_size
    ...
    def loss_and_gradients(
        self, x: np.ndarray, target: np.ndarray, gradient_clip: float
        ...
        for cache in reversed(caches):
            ...
            cell_total_gradient = (
                cell_gradient
                + hidden_gradient * output_gate * (1.0 - tanh_cell * tanh_cell)
            )
        ...
        global_norm = math.sqrt(
            sum(float(np.sum(value.astype(np.float64) ** 2)) for value in gradients.values())
        )
    ...
    def adam_update(
        ...
        for name, gradient in gradients.items():
            ...
            self.params[name] -= learning_rate * corrected_m / (
    ...
    def predict(self, x: np.ndarray, batch_size: int) -> np.ndarray:
        ...
        for start in range(0, len(x), batch_size):
            batch_prediction, _ = self.forward(
                x[start : start + batch_size], keep_cache=False
            )
...
def evaluate(actual: np.ndarray, prediction: np.ndarray) -> dict[str, Any]:
    ...
    result["mean_target_RMSE"] = float(
        np.mean([result[target]["RMSE"] for target in TARGETS])
    )
    return result
...
def main() -> None:
    ...
    print(
        f"sequences train={len(x_train_s):,}, validation={len(x_validation_s):,}, "
        f"test={len(x_test_s):,}; features={len(FEATURES)}; seq={args.sequence_length}"
    )
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, len(order), args.batch_size):
            ...
            batch_loss, gradients = model.loss_and_gradients(
                x_train_s[batch_indexes],
                y_train_s[batch_indexes],
                args.gradient_clip,
            )
        ...
        validation_prediction_s = model.predict(
            x_validation_s, args.batch_size
        )
        validation_loss = float(
            np.mean((validation_prediction_s - y_validation_s) ** 2)
        )
        row = {
            "epoch": epoch,
    ...
    validation_prediction = (
        model.predict(x_validation_s, args.batch_size) * normalization["y_std"]
        + normalization["y_mean"]
    )
    test_prediction = (
        model.predict(x_test_s, args.batch_size) * normalization["y_std"]
        + normalization["y_mean"]
    )
    ...
    prediction_frame["TA_actual"] = y_test[:, 0]
    prediction_frame["TA_pred"] = test_prediction[:, 0]
    prediction_frame["HM_actual"] = y_test[:, 1]
    prediction_frame["HM_pred"] = test_prediction[:, 1]
    prediction_frame["TA_error"] = (
        prediction_frame["TA_pred"] - prediction_frame["TA_actual"]
    )
    prediction_frame["HM_error"] = (
        prediction_frame["HM_pred"] - prediction_frame["HM_actual"]
    )
    ...
    metrics = {
        ...
        "best_epoch": int(pd.DataFrame(history)["validation_mse_scaled"].idxmin() + 1),
