...
이 프로젝트는 처음부터 최종 모델을 정해 놓고 시작하지 않았습니다. 세 명의 팀원이
처음 참여하는 기상 AI 대회였기 때문에, **데이터가 올바른가 → 검증이 믿을 만한가 →
새 모델이 실제로 일반화되는가 → 제출 환경에서 그대로 재현되는가**를 순서대로
해결하는 방식으로 발전했습니다.

| 단계 | 출발점과 문제 | 변경한 구조 | 결과와 판단 |
|---|---|---|---|
| 1. 문제 정의 | 위성영상 16채널로 96개 지점의 14시 TA·HM을 예측해야 함 | 공식 좌표·고도·날짜만 쓰는 정적 기준선과 별도 TA/HM 회귀 문제로 분리 | 허용 입력만 사용하는 최소 기준 확립 |
| 2. 수집 파이프라인 | API 실패, UTC/KST 혼동, 손상 파일과 재다운로드 위험 | 14:00 KST를 05:00 UTC로 변환하고, 캐시·재시도·완성도 검사·이어받기 구현 | 중단 후에도 정상 파일을 다시 받지 않는 수집 체계 완성 |
| 3. 위성 특징화 | 채널마다 해상도가 달라 같은 픽셀 좌표를 쓸 수 없음 | 공식 좌표 변환, 채널별 중심 픽셀, 결측 플래그와 주변 공간 통계 구축 | 날짜×지점 형태의 재현 가능한 학습 테이블 생성 |
| 4. 검증 개선 | 같은 날짜의 96개 행을 랜덤 분할하면 동일 위성영상이 양쪽에 섞임 | 날짜 그룹 분할과 연도순 rolling OOF, 마지막 연도 잠금 검증 도입 | 실제 평가 상황에 가까운 성능 비교 가능 |
| 5. 14시 모델 고도화 | 단일 모델과 중복 피처가 연도 변화에 불안정 | CatBoost를 중심으로 Ridge 잔차 보정, TA/HM 독립 피처 선택, 다중 seed 평균 적용 | TA와 HM에 서로 다른 최적 구조가 필요함을 확인 |
| 6. 공간·물리 피처 | 중심 픽셀만으로는 구름과 수증기의 주변 상태를 놓침 | 채널 차이, 동시간 지점 anomaly, 최근접 3지점 평균·편차를 HM 보조 CatBoost에 추가 | 2025 회고 점수 `2.389258 → 2.383245` |
| 7. 시계열 LSTM | 14시 한 장만으로 12~14시 변화 흐름을 표현하기 어려움 | 5시점 핵심 8채널 direct/residual LSTM과 기존 모델의 앙상블 비교 | LSTM 단독·잔차형은 기각, TA direct LSTM 20%는 조건부 개선 확인 |
| 8. 제출 시스템 | 학습 코드가 좋아도 Kaggle 재실행·파일 형식·규정 감사에서 실패할 수 있음 | 동결 모델 번들, 공식 노트북 셀 보존, 672행·결측·ID 순서·API 경로 자동 감사 | 실제 제출에서 3점대 진입, 재현 가능한 최종 제출 흐름 완성 |

## 모델 구조의 변화

```text
공식 station_list + GK-2A LE1B/KO 16채널
                     │
                     ▼
       시간대 변환·캐시·재시도·파일 검증
                     │
                     ▼
     채널별 지점 픽셀 + 날짜·태양·결측 특징
                     │
          ┌──────────┴──────────┐
          ▼                     ▼
     TA 기본 분기            HM 기본 분기
 CatBoost + Ridge         base CatBoost
          │                     │
          ▼                     ▼
  12~14시 평균 기반       물리·공간 CatBoost
  residual Ridge 보정     보조 분기 앙상블
          └──────────┬──────────┘
                     ▼
         다중 seed 평균·범위 제한
                     │
                     ▼
       동결 모델 번들 + Kaggle 추론 노트북
                     │
                     ▼
       7일 × 96지점 = 672행 제출 감사
```

LSTM은 전체 모델을 대체하는 최종 해답으로 취급하지 않았습니다. 과거 6월
rolling OOF에서는 `TA = 기존 모델 80% + direct LSTM 20%`가 개선됐지만, 단독 LSTM은
기존 모델보다 나빴고 HM과 residual LSTM은 안정성이 부족했습니다. 따라서 시계열 모델은
검증된 날짜 범위와 저장된 체크포인트가 있을 때만 켜는 **선택적 보조 분기**로 남겼습니다.

## 최종 결과와 현재 판단

서로 다른 시점의 결과를 섞어 과장하지 않도록 공식 제출과 로컬 검증을 구분합니다.

| 구분 | 결과 | 해석 |
|---|---:|---|
| 실제 Kaggle 제출 | 3점대 진입 | 대화에서 확인된 공식 제출 성과. 정확한 점수는 저장소 기록에 없어 임의로 쓰지 않음 |
| 2025 TA/HM 잠금 검증 | `2.443604` | 최근 OOF TA 보정 + HM residual 구조의 과거 홀드아웃 결과 |
| TA 피처 최적화 3-seed 평균 | `2.480033 → 2.428089` | 23개 TA residual Ridge 피처로 평균 점수 `0.051944` 개선 |
| HM 공간 보조 분기 3-seed 평균 | `2.389258 → 2.383245` | HM RMSE `7.208087 → 7.147951`; 작지만 모든 seed에서 개선 |
| 6월 TA LSTM 20% | OOF `3.874165 → 3.850346` | direct LSTM 단독이 아닌 저비중 앙상블에서만 개선 |
| 6월 TA LSTM 20% | 2025 잠금 `3.633517 → 3.535354` | 조건부 유망 결과이며 2026 공식 점수를 보장하지 않음 |

최종적으로 얻은 가장 중요한 결론은 다음과 같습니다.

- TA와 HM은 같은 피처와 같은 모델을 쓰기보다 별도 분기로 최적화하는 편이 좋았습니다.
- TA는 14시 CatBoost를 중심으로 Ridge 잔차 보정과 제한적인 시계열 앙상블이 유효했습니다.
- HM은 LSTM보다 동시간 채널 차이와 주변 지점 공간 정보가 더 안정적이었습니다.
- 복잡한 피처를 많이 넣는 것보다 rolling OOF에서 반복 개선된 작은 피처 묶음만 남겼습니다.
- 2025는 마지막 회고 평가로만 사용했으며, 그 결과를 보고 가중치를 다시 고르지 않았습니다.
- 최종 추론에서는 ASOS를 읽거나 모델을 재학습하지 않고 GK-2A와 공식 지점 정보만 사용합니다.

## 실험 계보와 보존 위치

주요 실험은 목적별 브랜치에 보존되어 있습니다.

| 브랜치 | 역할 |
|---|---|
| [`agent/colab-drive-nc-preprocessing`](../../tree/agent/colab-drive-nc-preprocessing) | Drive 기반 NC 전처리와 최초 LSTM 기준선 |
| [`agent/shortterm-12to14-pipeline`](../../tree/agent/shortterm-12to14-pipeline) | 12~14시 10분 간격 수집·이어받기·연도별 데이터 구축 |
| [`agent/ta-hm-oof-calibrated`](../../tree/agent/ta-hm-oof-calibrated) | TA OOF 보정과 HM residual 기준선 |
| [`agent/feature-engineering-optimizer`](../../tree/agent/feature-engineering-optimizer) | 단계적 피처 추가·제거와 TA 23개 피처 확정 |
| [`agent/team-catboost-feature-ensemble`](../../tree/agent/team-catboost-feature-ensemble) | HM 물리·공간 CatBoost 보조 분기 |
| [`agent/june-lstm-feasibility`](../../tree/agent/june-lstm-feasibility) | 6월 direct/residual LSTM 채택 검증 |
| [`agent/2026-ta-lstm-blend-submission`](../../tree/agent/2026-ta-lstm-blend-submission) | TA LSTM 20% 제출 실험과 공식 형식 노트북 |
| [`agent/submission-2026-end-to-end`](../../tree/agent/submission-2026-end-to-end) | 동결 모델 번들·Kaggle 추론·제출 계약 감사 |

`main`은 공통 전처리와 규정 준수 기반을 설명하고, 위 브랜치들은 실험 당시 코드·노트북·
보고서를 그대로 보존합니다. 이 구조 덕분에 최종안뿐 아니라 어떤 시도가 채택되거나
기각됐는지도 추적할 수 있습니다.
